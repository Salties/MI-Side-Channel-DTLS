...
int IsDtlsConnected(dtls_context_t * ctx, session_t * session)
{
    ...
    if (!peer)
	return 0;
    return (peer->state == DTLS_STATE_CONNECTED ? 1 : 0);
}
...
#ifdef DTLS_ECC
static int get_ecdsa_key(struct dtls_context_t *ctx, const session_t * session, const dtls_ecdsa_key_t ** result)
{
...
verify_ecdsa_key(struct dtls_context_t *ctx,
		 ...
		 const unsigned char *other_pub_x, const unsigned char *other_pub_y, size_t key_size)
{
    return 0;
}
#endif				/* DTLS_ECC */

static void try_send(struct dtls_context_t *ctx, session_t * dst, char *buf, int len)
{
    //int res;
...
#define MAX_READ_BUF 2000
    ...
    ret = dtls_handle_message(ctx, &session, buf, len);
    connected = IsDtlsConnected(ctx, &session);
