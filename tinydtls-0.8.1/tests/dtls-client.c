...
get_ecdsa_key (struct dtls_context_t *ctx,
	       const session_t * session, const dtls_ecdsa_key_t ** result)
{
...
static void
try_send (struct dtls_context_t *ctx, session_t * dst, char *buf, int len)
{
  //int res;
...
main (int argc, char **argv)
{
  ...
  /* use port number from command line when specified or the listen
     port, otherwise */
  dst.addr.sin.sin_port =
    htons (atoi (optind < argc ? argv[optind++] : port_str));
