...
\cite{6lpRplAtk} gives an overview of attacks that targets 6LoWPAN and RPL. In general, lack of authentication in ICMP messages poses a great vulnerability in WSN. A malicious 

%%6LoWPAN Security Overview
%\cite{6lowpanSec} provides a security review for the same protocol suite. Specifically, it concludes several attacks at different layers:
%\begin{itemize}
%	\item At Physical Layer, jamming attacks can interrupt the radio communication can tampering attacks can compromise the cryptographic materials of a node.
%	\item At MAC Layer, CSMA/CA can be exploited to conduct DoS attacks by injecting spoofing frames to create collisions. Spoofed unauthenticated frames can trigger the responses of a victim and therefore deplete the battery.
%	\item Packets without authentication poses a great vulnerability in Network Layer. Sinkhole\cite{Sinkhole} attack disables the network by sending for false routing message. Hello flood\cite{HelloFlood} disables the network by flooding Hello message, causing battery depletion.
%\end{itemize} 
