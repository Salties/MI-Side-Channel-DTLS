...
As of this project we consider only information leakage within one Session. One might argue that there is potential information leakage when jointly analyse traces from different Sessions, such as the frequency of Requests can ``leak'' itself. We do not take these leakage into concern as it does not make much sense without considering within a higher layer application.
...
Also in our experiments, we make a reasonable assumption based on our code that each Session is independent, as each execution of the code is independent to its last execution.
...
\subsection{Cross Reference of Observables with Known Attack}
...
\begin{description}[style=nextline]
	\item[Direction]
	The same feature can be observed in the observables by the source and destination address in IPv6 Headers.
