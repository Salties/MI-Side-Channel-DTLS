...
%Fix vs Random
To verify whether the execution time leaks any information of the AES key, we applied the Fixed vs Random strategy inspired by TVLA\cite{TVLA}, with slight adaptation. To be more specifically, we collect timing samples for fixed key and fixed plaintext, and compares it to random keys and random plaintext, using t-test\footnote{The timings are assumed to follow a normal, or similar, distribution.} to determine whether a statistical difference can be observed  between two groups. The test is to see whether a specific input to AES, incarnated by the fixed group, has any bias comparing to average case, incarnated by the random group. We use the null hypothesis that two groups of AES timing data have the same mean for the t-test. Using the t-score threshold adopted by \cite{TVLA}, $4.5$, if the t-test returns a t-score which absolute value is greater than the threshold, we conclude there is a potential leakage of the key from the execution time; otherwise we conclude no leakage is detected.

%Calibration
However during the experiments, we realised that a constant timing error can be induced by codes outside the timing area on platform specific manner, e.g. a memcpy call to initialise some blocks of data in the memory on CC2538 may reduce the execution time of the timed code, but the same phenomenon is not observed on TelosB. The cause of this error is unclear so far, but we have ruled out the following factors:
...
In order to mitigate the error, calibrations are needed before measurements. To calibrate the tests, we replace AES encryption for two groups by code of constant execution time as an indicator of calibration. If the execution time of the replaced code in two groups have similar results, then we consider the tests are calibrated and replace the AES encryption back to begin the tests.
...
\begin{description}
	...
	\item[CC2538] On CC2538, our tests are calibrated by reading out the whole plaintext once before each measurement. This measure applied on TelosB causes timing error.
\end{description}

In our experiments, we calibrated using $2000$ random\_rand() calls to $\pm{2}$ ticks.

%Test result
We tested this method for  $n \in \{ 50, 100, 150, 200\}$ executions respectively with random keys and random plaintext to estimate the average execution time of the AES implementations. The result is shown in \Cref{Tbl: AES execution time estimation of Contiki}. The measurements are done in system real timer clock ticks, with $32768$ ticks per second, i.e. $3.05 * 10^{-5}$s per tick. We estimate the average time of one AES execution by ordinary least square estimation.
...
Nevertheless, our test result surprisingly indicates that there is a potential leakage in the AES execution time on two implementation we tested, as shown in \Cref{Tbl: Fixed vs Random test on Contiki AES timing}.
...
According to the $4.5$ t-score threshold set by TVLA, only CC2538 software AES has passed this test. The other results indicates the execution time could be a potential leakage source of key in TelosB implementations. However, we have not yet came up with a reasonable explanation of the cause of this variance. We leave this as an open question in this report.

On the other hand, we also noticed that even for the most significantly varied platform, TelosB Software AES implementation, the range is 139 ticks for $200$ executions, which translates to $0.0212$ms per execution. We consider such variance not applicable in the captured traffic as many other factors, such as  RDC and CSMA/CA, can induce significantly greater noises to the timing.
