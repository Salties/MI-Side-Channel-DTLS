...
\cite{802154sec} analysed some design problems of 802.15.4 Security and  we summarised them in \Cref{Subsec: 802154 Sec Issue}. Since noncoresec supports only network shared key and does not implement ACL, we key management issues are not concerned in our settings. Instead, two potential problems are investigated in this report:
...
%NS & NA, DIS & DIO
NA is only sent as a response to NS; therefore if a round trip of two 87 bytes unicast packet is observed, it is likely that the first one is NS and the second NA. Similarly, DIS is sent for requesting DIO messages.  
...
Intuitively, one of the most important character of the distributions shown in \Cref{Fig: Histograms of ECHO Response Times} is that the ECHO response times are densely clustered near a specific value for each distribution, while a few of them are drifted far away. Therefore in addition to the means of ECHO response times, we also use their median to characterise their distributions, as summarised in \Cref{Tbl: ICMP ECHO Response Time}.
...
\begin{table}[ht!]
	...
	\end{tabular}
	\caption{ICMP ECHO Response Time}
	\label{Tbl: ICMP ECHO Response Time}
\end{table}

This result is consistent to our expectation, as CC2538 has much better performance than TelosB, even though the hardware AES can greatly improve the performance, as shown in \Cref{Tbl: AES execution time estimation of Contiki}.

It is apparent that TelosB with the software AES implementation takes an obvious longer time than the other settings. To further verify whether it is distinguishable between CC2538 with software AES implementation and TelosB with hardware AES implementation, we tried the data with two sample t-test and two sample Kolmogorov-Smirnov test, KS test, respectively. For simplicity, we denote $T_C}$ and $T_T}$ to be the ECHO response time for CC2538 software AES and TelosB hardware AES respectively. The test results are shown in \Cref{Tbl: p-values for statistical tests over ECHO response time}.

\begin{table}[ht!]
	\center
	\begin{tabular}{|c|c|c|}
	\hline
	        & $T_C == T_T$ & $T_C < T_T$ \\ \hline
...
The test results suggest that the ECHO response time for CC2538 software AES is likely to be distinguishable from TelosB hardware AES, with the CC2538 tends to respond faster than TelosB.

Therefore in conclusion, we suggest that the ECHO response time can be potentially exploited to distinguish the platforms and settings in our experiments.
...
\Cref{Tbl: Packet Features of ICMPv6 Messages in 6LoWPAN with noncoresec}, the padding can be no more than $42$ bytes for a single frame, which is nearly below $1/3$ overhead. To cope with the current standard, the padding can be done using the Private experimentation type numbers\cite{rfc4443} of ICMPv6, where the receiver can simply silently discard the paddings.
