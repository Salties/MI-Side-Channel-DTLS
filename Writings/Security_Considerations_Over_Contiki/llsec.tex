...
Recalling \Cref{Eq: broadcast llsec data size} and \Cref{Eq: unicast llsec data size}, the minimum sizes of application packets, i.e. $l_D = 1$, are 88 bytes and 102 bytes respectively; therefore packets with size less than these values can immediately categorised as non application packets, which are basically ICMPv6 messages in practice. Therefore, some of these ICMPv6 messages can be distinguished in the traffic. We  represent the frame size and destination address combination in a two dimension vector $<l, dst>$ where $l$ is the size of frame in bytes and $dst \in \{\text{unicast}, \text{broadcast}\}$.
...
Specifically,
\begin{enumerate}
	\item If a trace of $<<87,unicast>, <87, unicast>>$ is observed, then the first packet is a NS message and the second a NA message.
	\item If 
 \end{enumerate}
