...
\begin{itemize}
	...
	\item Medium $t_A$, for $t_A \in [1.5, 100)$ ms.
	\item High $t_A$, for $t_A \geq 100$ ms.
\end{itemize}
...
and \\
\url{https://github.com/Salties/MyRepository/tree/master/experiments/dtlsapptime/Data}
...
The result in \Cref{Fig: Estimation for low tA} indicates that the estimation totally failed at low $t_A$, such that $\hat{t}_A$ are negative values. We consider this is caused by the fact that at such level the execution time is dominated by the systematic noise. At this level of $t_A$, $\epsilon$ increases nearly linearly whilst the $\mu$ decreases nearly inverse proportionally with $t_A$.
...
$\hat{t}_A$ increase nearly linearly with $t_A$ in \Cref{Fig: Estimation for medium tA}. However, we realised that our estimations are strictly beneath the actual value. The $\epsilon$ appears to be random at these values but its range is within $[1.721, 1.860]$ ms in our experiments. Due to the narrow range of $\epsilon$, $\mu$ decreases dramatically as $t_A$ increases.

Applying least squares fitting for medium $t_A$ and $\hat{t}_A$, we have an estimation of:
\begin{equation}
	t_A = 1.830 + 0.999\hat{t}_A 
\end{equation}

where $R^2=1.000$, which indicates that our estimation $\hat{t}_A$ is almost perfectly linearly related to $t_A$. The sample mean and standard deviation of $\epsilon$ are $1.784$ and $0.049$ ms respectively.

The result suggests that for medium $t_A$, the accuracy of estimation increases as $t_A$ increases. Specifically, our data suggests that for $t_A > 35.7$ ms, we can have the relative approximation error $\mu < 0.05$.

\subsubsection{High Execution Time Estimation}

Further increasing $t_A$, we have the result for high $t_A$ ($\geq 100$ ms), shown in \Cref{Fig: Estimation for high tA}.
...
The result in \Cref{Fig: Estimation for high tA} is similar to the result of medium $t_A$ such that $\hat{t}_A$ is linear to $t_A$. The least square estimation is:
\begin{equation}
	t_A = 1.569 + 1.000\hat{t}_A
\end{equation}

where $R^2 = 1.000$. 

The absolute approximation error $\epsilon$ for high $t_A$ has sample mean and standard deviation of $1.660$ and $0.094$ respectively, with range $[1.571, 1.898]$ ms.

The relative approximation error $\mu$ further decreases in this group, reaching $< 0.01$ when $t_A$ is about $160$ ms.

\subsubsection{Estimation Errors}

During the experiments, we realised that the estimation error $e$, defined as $e = t_A - \hat{t}_A$, is constantly positive in all our experiments, as shown in
\begin{figure}
	\center
