...
\begin{itemize}
	\item DTLS is built on datagrams instead of TCP like TLS. In a 6LoWPAN network, this is effectively UDP. As a result DTLS implemented a simple reliable transmission mechanism during handshake to synchronise the cryptographic parameters.
	\item Explicit sequence number and epoch value on each DTLS record. We explain theses fields in following context.
	...
	\item Stateful ciphersuites are disabled\cite{DtlsCiphers} such as RC4, as maintaining states is difficult on UDP.
\end{itemize}
%We ignore handshake
%DTLS record format
