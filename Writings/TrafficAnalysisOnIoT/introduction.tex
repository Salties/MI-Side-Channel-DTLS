...
The paper is structured as follows. We first review the related literatures in \Cref{RelatedWork} and briefly report a security flaw in Contiki source code in \Cref{noncoresec}. We then present the first example that extracts ICMP messages in \Cref{ICMPAttack}, followed by another example that reveals hardware information in \Cref{Sec:DistinguishDevice}. \Cref{PingLoad} proposes a new type of side channel attack, we named PingLoad, that fingerprints the application running on an IoT node. Finally we conclude the paper in \Cref{conclusion}.
...
%To our knowledge, this is the first proposal of such type of our fingerprint attack in \Cref{PingLoad}.